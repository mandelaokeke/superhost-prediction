...
The project was originally completed during a master's program and has been reframed as a transparent portfolio case study. Its value is in connecting a classification model to a practical prioritization workflow—not in claiming that observational correlations cause future conversion.
...
The boosted-tree model is the current champion. It exceeds the historical ANN's 0.896 ROC-AUC despite the stricter unseen-host evaluation. See the full [`benchmark report`](reports/benchmark.md).
